...
-   Sequence data processed with SnpArcher pipeline with the following [sample table](Data/sample_210.csv), [config.yaml](Data/snparcher_config.yaml) and [config_slurm.yaml](Data/slurm_config.yaml)
...
-   The GATK best practices filters (removing all indels, non-biallelic SNPs, SNPs with a minor allele frequency \< 0.01, SNPs with \>75% missing data, and samples with \<2x sequencing depth) were then applied leaving **74407954 SNPs**.

-   The approximate nucleotide diversity in the sample using the Watterson estimator is 1.3%.

-   The quality control analysis from SnpArcher considers randomly selected SNPs within a set window size to end up with approximately 100k SNPs (in this QC report, 100199). These are effectively an LD pruned set of SNPs. All analyses in this report are based on this set of 100k SNPs.

[SnpArcher QC analyses](Data/cuc_2_1_qc.html) revealed two outliers (samples 57 and 127) that correspond to unique color-morph individuals from the northern region of the Archipelago (red and black; first record of these color morphs for the Galapagos). We corroborated species identity of these two outliers by extracting the COI and BLAST, which called the right species *I. fuscus* with 99.66% identity.

![Quality control PCA from SnpArcher identifying two outliers.](Data/outliers_pca.png)
...
eigenValues <- read_delim("./Data/CU_Ifusc_2_1_filtered_57_127_biallelic_snp_pca_results.eigenval", delim = " ", col_names = F)

eigenVectors <- read_delim("./Data/CU_Ifusc_2_1_filtered_57_127_biallelic_snp_pca_results.eigenvec", delim = "\t", col_names = T)
...
ggplot(data = eigen_env,mapping = aes(x = PC1, y = PC2, color = Bioregion)) +
    ...
    labs(title = "PCA Full data - No Outliers (57 & 127) - Biallelic SNPs (27006852)",
...
# August 28 To DO: ##
#NOW make a Manhattan plot but color by LD value or whether each SNP surpasses an LD threshold

# Also make a PCA plot of Chromosome 21, 22, 3 for the top loading SNPs and also Manhattan plots for each chromosome (full data or subsample 1 in a thousand)
```

```{r}
#PCA high loadings Chroms 3, 21, 22

eigenValues <- read_delim("./Data/high_load_snps_chr3_pca.eigenval", delim = " ", col_names = F)

eigenVectors <- read_delim("./Data/high_load_snps_chr3_pca.eigenvec", delim = "\t", col_names = T)
...
#Plot PCA
ggplot(data = eigen_env,mapping = aes(x = PC1, y = PC2, color = PC_Cluster)) +
    geom_point(size = 2, alpha=0.7,show.legend = T ) +
    geom_hline(yintercept = 0, linetype="dotted") +
    #geom_text(hjust=0, vjust=0,aes(label=IID)) +
    geom_vline(xintercept = 0, linetype="dotted") +
    theme(legend.position="right") +
    labs(title = "PCA Chr 3 - high loading variants (N = 2096)",
         x = paste0("Principal component 1 (",eigen_percent[1,1]," %)"),
         y = paste0("Principal component 2 (",eigen_percent[2,1]," %)"))+
    theme_minimal()
...
```{r}
#PCA high loadings Chroms 21

eigenValues <- read_delim("./Data/high_load_snps_chr21_pca.eigenval", delim = " ", col_names = F)

eigenVectors <- read_delim("./Data/high_load_snps_chr21_pca.eigenvec", delim = "\t", col_names = T)
...
ggplot(data = eigen_env,mapping = aes(x = PC1, y = PC2, color = PC_Cluster)) +
    ...
    geom_vline(xintercept = 0, linetype="dotted") +
    theme(legend.position="right") +
    labs(title = "PCA Chr 21 - high loading variants (N = 4618)",
         x = paste0("Principal component 1 (",eigen_percent[1,1]," %)"),
         y = paste0("Principal component 2 (",eigen_percent[2,1]," %)"))+
    theme_minimal()
```
...
```{r}
#PCA high loadings Chroms 22

eigenValues <- read_delim("./Data/high_load_snps_chr22_pca.eigenval", delim = " ", col_names = F)

eigenVectors <- read_delim("./Data/high_load_snps_chr22_pca.eigenvec", delim = "\t", col_names = T)
...
ggplot(data = eigen_env,mapping = aes(x = PC1, y = PC2, color = PC_Cluster)) +
    ...
    geom_vline(xintercept = 0, linetype="dotted") +
    theme(legend.position="right") +
    labs(title = "PCA Chr 22 - high loading variants (N = 2016)",
         x = paste0("Principal component 1 (",eigen_percent[1,1]," %)"),
         y = paste0("Principal component 2 (",eigen_percent[2,1]," %)"))+
    theme_minimal()
```
...
-   Bcftools does a genome-wide pruning with 50kb windows and keeps a random SNP. Heavily reduced the number of SNPs (N = 551), initial clustering pattern disappears.
...
-   PLINK2 prunes on each chromosome and maintains an SNP even if the chromosome is smaller than 50Kb (SNPs = 16816). More SNPs maintain cluster pattern.
...
-   Genome-wide LD analysis. LD was calculated for 0.1% of the top-loading SNPs (N = 27007).
...
## 8.1 LD chromosomes 3, 21, 22 - Full data

``` bash
#workdir 
/lustre1/home2/nt246_0001/jdo53/CU_Ifusc_2_1_Analyses/LD_per_Chr

#Plink file for genome
/lustre1/home2/nt246_0001/jdo53/CU_Ifusc_2_1_Analyses/PCA/CU_Ifusc_2_1_filtered_57_127_polymorphic_snp_plink


#Create plink files for each chromosome
# Edit this to your base dataset prefix (BED/BIM/FAM)
BASE=/lustre1/home2/nt246_0001/jdo53/CU_Ifusc_2_1_Analyses/PCA/CU_Ifusc_2_1_filtered_57_127_polymorphic_snp_plink
...
#Intra‑chromosomal LD with PLINK 1.9
#Compute within‑chromosome r² in sliding windows (here 1 Mb) and write compressed LD pair lists with positions for plotting; set --ld-window-r2 0 to keep all pairs in the window, or raise it (e.g., 0.2) for lighter output.

for c in 3 21 22; do
/programs/plink-1.9-x86_64-beta7/plink \
    --bfile chr${c}_subset \
    --r2 with-freqs gz \
    --ld-window-kb 1000 --ld-window 999999 --ld-window-r2 0.2 \
    --out chr${c}_ld1mb
done
```

```{r}
#| label: LD_per_chromosome
#| eval: false
#| include: true
#LD per chromosome
setwd("/home2/jdo53/CU_Ifusc_2_1_Analyses/LD_per_Chr")

suppressPackageStartupMessages({ library(data.table); library(ggplot2); library(scales) })

#plot_local_ld <- function(ld_path, out_png, chr_label, local_window_kb = 100) {
plot_local_ld <- function(ld_path, chr_label, local_window_kb = 100) {
  if (!file.exists(ld_path)) stop(sprintf("LD file not found: %s", ld_path))
  # Read .ld or .ld.gz
  cmd <- if (grepl("\\.gz$", ld_path)) sprintf("zcat %s", shQuote(ld_path)) else ld_path
  dt <- fread(cmd = cmd)
  if (nrow(dt) == 0) stop("LD file is empty; re-run PLINK with --r2 with-freqs and nonzero windows.")
  # Standardize names to lower case and verify expected columns
  setnames(dt, tolower(names(dt)))
  need <- c("chr_a","bp_a","snp_a","chr_b","bp_b","snp_b","r2")
  if (!all(need %in% names(dt))) stop("Missing columns in LD file; ensure PLINK 1.9 --r2 with-freqs output.")
  # Within-chromosome and local window
  dt <- dt[chr_a == chr_b]
  dt[, dist := abs(bp_b - bp_a)]
  dt <- dt[dist <= local_window_kb*1000]
  a <- dt[, .(SNP = snp_a, BP = bp_a, r2 = r2)]
  b <- dt[, .(SNP = snp_b, BP = bp_b, r2 = r2)]
  long <- rbind(a, b)
  per <- long[, .(BP = max(BP), localMeanR2 = mean(r2, na.rm=TRUE), maxR2 = max(r2, na.rm=TRUE), N = .N), by = SNP]
  setorder(per, BP)
  
  p <- ggplot(per, aes(x = BP, y = localMeanR2, color = maxR2)) +
    geom_point(size = 0.6, alpha = 0.9) +
    scale_color_gradientn(
      colours = c("#00007F","#0000FF","#007FFF","#00FFFF","#7FFF00","#FFFF00","#FF7F00","#FF0000"),
      limits = c(0,1), oob = squish, name = expression(max~r^2)
    ) +
    labs(
      title = sprintf("Mean r² (%dkb) — %s", local_window_kb, chr_label),
      x = "Position (bp)", y = expression(Mean~r^2)
    ) +
    theme_minimal(base_size = 12) +
    theme(panel.grid.minor = element_blank(), legend.key.height = unit(36,"pt"), plot.title = element_text(face="bold"))
  print(p)
  #ggsave(out_png, p, width = 10, height = 5, dpi = 300) #this is to save plot as png
}

# Correct calls with the filenames produced by the earlier PLINK commands
# plot_local_ld("chr3_ldpairs_1mb.ld.gz",  "Chr3_localLD.png",  "Chr3") #this is the correct way if using the function to also save as png
# plot_local_ld("chr21_ldpairs_1mb.ld.gz", "Chr21_localLD.png", "Chr21")
# plot_local_ld("chr22_ldpairs_1mb.ld.gz", "Chr22_localLD.png", "Chr22")

plot_local_ld("chr3_ld1mb.ld.gz", "Chr3") #this is only to view ggplot
plot_local_ld("chr21_ld1mb.ld.gz", "Chr21")
plot_local_ld("chr22_ld1mb.ld.gz", "Chr22")

```

![](images/LD_r2_chrom3_manhattan.png)

![](images/LD_r2_chrom21_manhattan.png)

![](images/LD_r2_chrom22_manhattan.png)
...
-   There is an outlier point that does not correspond to a high loading window and located in Chr20.
...
To facilitate the visualization of the position of the SNPs across the genome, we will create Manhattan plots of the per-site mean depth per cluster, and highlighting in red the high loading windows.
...
-   Again here we can clearly identify that the outlier for the Left Cluster corresponds to a window in the Chr20.
